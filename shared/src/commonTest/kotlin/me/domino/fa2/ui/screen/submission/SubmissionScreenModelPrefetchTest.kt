...
class SubmissionScreenModelPrefetchTest {
    ...
    fun computesPrefetchWindowIndices() {
        assertEquals(listOf(1, 2, 3), computeSubmissionPrefetchIndices(currentIndex = 0, lastIndex = 3))
        assertEquals(listOf(3, 4, 5, 1), computeSubmissionPrefetchIndices(currentIndex = 2, lastIndex = 5))
        assertEquals(listOf(4), computeSubmissionPrefetchIndices(currentIndex = 5, lastIndex = 5))
    }

    @Test
    fun prefetchesNext3AndPrevious1AfterDebounce() = runTest(dispatcher.scheduler) {
