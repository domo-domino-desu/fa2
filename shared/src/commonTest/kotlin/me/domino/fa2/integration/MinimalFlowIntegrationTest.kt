...
class MinimalFlowIntegrationTest {
    ...
    /**
     * 手动输入 cookie 时会过滤 Cloudflare 项，仅保留 auth 项；
     * WebView 获得的 Cloudflare cookie 会被保留。
     */
    @Test
    ...
    fun setCookieMergePersistsAndDeletesAcrossRestart() = runTest {
        ...
        cookiesStorage.mergeSetCookieValues(
            listOf(
                "a=1; Path=/; HttpOnly",
                "cf_clearance=cloud; Path=/; Secure",
            ),
        )
...
private fun testOverrideModule(
    ...
    single<AppDatabaseBuilderFactory> {
        AppDatabaseBuilderFactory {
            val dbPath = "/tmp/fa2-test-${Clock.System.now().toEpochMilliseconds()}-${Random.nextInt(100_000)}.db"
    ...
    single<DataStore<Preferences>> {
        PreferenceDataStoreFactory.createWithPath(
            produceFile = { storage.dataStorePath },
        )
    }
    single(named(KOIN_QUALIFIER_COOKIE_VAULT)) {
        KSafe(fileName = storage.cookieVaultFileName)
    }
}
...
private fun newTestStorageConfig(): TestStorageConfig {
    ...
    return TestStorageConfig(
        dataStorePath = "${FileSystem.SYSTEM_TEMPORARY_DIRECTORY}/fa2-test-$randomSuffix.preferences_pb".toPath(),
        cookieVaultFileName = "fa2test_$randomSuffix",
    )
}

private data class TestStorageConfig(
    val dataStorePath: Path,
