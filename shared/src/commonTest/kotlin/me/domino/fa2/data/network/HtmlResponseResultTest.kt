...
class HtmlResponseResultTest {
    ...
    fun detectsCloudflareChallenge() {
        val result = HtmlResponseResult.classify(
            statusCode = 403,
            headers = mapOf(
                "Server" to listOf("cloudflare"),
                "CF-Ray" to listOf("abc123"),
            ),
    ...
    fun doesNotMisclassifyMaturePageAsChallenge() {
        val matureHtml = TestFixtures.read("www.furaffinity.net:view:60245416-mature-content-message.html")
        val result = HtmlResponseResult.classify(
            statusCode = 200,
            headers = mapOf(
                "Server" to listOf("cloudflare"),
