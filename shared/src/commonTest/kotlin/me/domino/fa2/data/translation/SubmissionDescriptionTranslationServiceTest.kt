...
class SubmissionDescriptionTranslationServiceTest {
    ...
    fun extractsParagraphsFromPlainDescription() {
        ...
        assertTrue(blocks.any { block -> block.sourceText.contains("Feed me with coffee", ignoreCase = true) })
        assertTrue(blocks.none { block -> block.sourceText.startsWith('\n') || block.sourceText.endsWith('\n') })
    }
    ...
    fun extractsParagraphsInsideCodeWrapperAndKeepsWrapperTag() {
        ...
        assertTrue(blocks.size >= 4, "Expected multiple paragraphs in code wrapper, actual=${blocks.size}")
        assertTrue(blocks.any { block -> block.sourceText.contains("Since I thought", ignoreCase = true) })
        assertTrue(blocks.any { block ->
            block.sourceText.contains(
    ...
    fun doesNotLeakFullCodeContentIntoEachSplitBlock() {
        ...
        assertTrue(
            blocks.none { block ->
                block.originalHtml.contains("HEADER LINE") &&
                        block.originalHtml.contains("FOOTER LINE")
            },
    ...
    private fun createService(): SubmissionDescriptionTranslationService {
        ...
        val tempPath = "${FileSystem.SYSTEM_TEMPORARY_DIRECTORY}/fa2-test-$randomSuffix.preferences_pb".toPath()
        ...
        val translationPort = object : TranslationPort {
