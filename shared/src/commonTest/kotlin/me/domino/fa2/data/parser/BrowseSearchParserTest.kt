...
class BrowseSearchParserTest {
    ...
    fun parsesBrowseListingAndFormBasedNextPage() {
        ...
        val page = parser.parseListing(
            html = html,
            baseUrl = FaUrls.browse(),
        )
    ...
    fun parsesSearchListingAndAnchorNextPage() {
        val html = TestFixtures.read("www.furaffinity.net:search:wolf-keywords-female-trans_female.html")
        val parser = GalleryParser()
        val searchUrl = FaUrls.search(
            FaUrls.SearchParams(
                q = "wolf @keywords female trans_female",
            ),
        )
