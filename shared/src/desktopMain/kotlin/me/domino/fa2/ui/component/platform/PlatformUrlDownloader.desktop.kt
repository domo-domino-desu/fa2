...
private suspend fun downloadUrlWithSaveDialog(
    ...
    val target = runCatching {
        chooseSaveFile(defaultFileNameForUrl(normalized))
    }.getOrNull() ?: return false
    return withContext(Dispatchers.IO) {
        runCatching {
            target.parentFile?.mkdirs()
            val connection = URI(normalized).toURL().openConnection().apply {
...
private fun chooseSaveFile(defaultFileName: String): File? {
    return runOnAwtEventThread {
        val owner = Frame()
        try {
            FileDialog(owner, "保存文件", FileDialog.SAVE).apply {
