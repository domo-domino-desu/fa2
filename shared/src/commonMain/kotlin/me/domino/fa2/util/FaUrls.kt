    ...
    fun search(params: SearchParams): String {
        val query = buildList {
            ...
            if (params.range == "manual") {
                if (params.rangeFrom.isNotBlank()) add("range_from=${params.rangeFrom.encodeURLParameter()}")
                if (params.rangeTo.isNotBlank()) add("range_to=${params.rangeTo.encodeURLParameter()}")
            }
