...
object ParserUtils {
    ...
    private val submissionDataScriptRegex = Regex(
    ...
    /**
     * 根据原图 URL 推导缩略图 URL。
     * 需要 submission sid 与原图路径中的时间戳目录（`/art/{author}/{timestamp}/...`）。
     */
    fun deriveSubmissionThumbnailUrlFromFullImage(
        ...
        val timestamp = fullImageTimestampRegex.find(pathOnly)
            ?.groupValues
            ?.getOrNull(1)
            ?.trim()
    ...
    private fun parseUrlHost(url: String): String? {
        ...
        val host = if (hostEnd >= 0) normalized.substring(hostStart, hostEnd) else normalized.substring(hostStart)
    ...
    /**
     * 解析页面中提交项对应的作者头像地址。
     * 依赖页面内 `#js-submissionData` JSON，返回 `sid -> avatarUrl`。
     */
    fun parseSubmissionAvatarUrls(html: String): Map<Int, String> {
        ...
        return buildMap(capacity = root.size) {
            root.forEach { (sidRaw, entryElement) ->
                ...
                val avatarUrl = FaUrls.avatar(
                    usernameLower = usernameLower,
                    avatarMtime = avatarMtime,
                )
    ...
    private fun parseSubmissionDataRoot(html: String) = runCatching {
        val jsonText = submissionDataScriptRegex.find(html)
            ?.groupValues
            ?.getOrNull(1)
            .orEmpty()
            .trim()
