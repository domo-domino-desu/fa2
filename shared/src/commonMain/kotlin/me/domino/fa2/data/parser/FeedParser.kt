...
class FeedParser {
    /** submissions 卡片候选选择器。 */
    private val submissionSelectors = listOf(
        "#messagecenter-submissions figure",
    ...
    fun parse(
        ...
        val hasTagBlocklist = document.selectFirst("body")
            ...
            ?.trim()
            ?.isNotBlank() == true
        val figureNodes = submissionSelectors
            .asSequence()
        ...
        val submissions = figureNodes.mapNotNull { node ->
            parseSubmission(
                node = node,
        ...
        val nextPageUrl = document.select("a, form")
    ...
    private fun parseSubmission(
        ...
        val rawSubmissionUrl = node.selectFirst("a[href*=\"/view/\"]")
            ?.attr("href")
            .orEmpty()
