...
class WatchlistParser {
    ...
    private fun parseUsers(
        ...
        document.select("div.watch-list-items a[href*='/user/']")
            .forEach { anchor ->
    ...
    private fun parseWatchlistUser(
        anchor: Element,
        baseUrl: String,
    ): WatchlistUser? {
        ...
        val username = rawHref
            .substringAfter("/user/", "")
            .substringBefore('/')
            .trim()
            .ifBlank { null }
            ?: return null
        val displayName = anchor.selectFirst(".c-usernameBlockSimple__displayName")
            ?.text()
            ?.trim()
            .orEmpty()
            .ifBlank {
    ...
    private fun parseNextPageUrl(
        ...
        val nextForm = document.select("div.watchlist-navigation form")
            .firstOrNull(::isNextFormEnabled)
            ?: return null
        val action = nextForm.attr("action")
            .trim()
            .takeIf { value -> value.isNotBlank() }
            ?: return null
        val absoluteAction = ParserUtils.toAbsoluteUrl(baseUrl, action)
        val nextPage = nextForm.selectFirst("input[name=page]")
            ?.attr("value")
            ?.trim()
            ?.takeIf { value -> value.isNotBlank() }
