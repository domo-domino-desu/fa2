...
class GalleryStore(
    ...
    suspend fun loadPageOnce(
        ...
        stream(section, username, nextPageUrl)
            .first { state -> state !is PageState.Loading }
    ...
    private fun buildStore(): Store<SectionPageKey, GalleryPage> {
        val fetcher = Fetcher.of<SectionPageKey, GalleryPage>(name = "gallery-sections-fetcher") { key ->
            when (key.section) {
        ...
        val sourceOfTruth = SourceOfTruth.of<SectionPageKey, GalleryPage, GalleryPage>(
            reader = { key ->
    ...
    private fun parseSectionPageKey(
        section: Section,
        cacheKey: String,
    ): SectionPageKey? {
...
private fun Flow<PageState<GalleryPage>?>.flowWithInitialLoading(): Flow<PageState<GalleryPage>> = flow {
