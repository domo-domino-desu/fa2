...
class GalleryParser {
    ...
    private val figureSelectors = listOf(
    ...
    fun parse(
        ...
        val submissions = parseSubmissions(
            html = html,
            baseUrl = baseUrl,
    ...
    private fun parseSubmissions(
        ...
        val profileAvatarUrl = document.selectFirst("userpage-nav-avatar img")
            ?.attr("src")
            ?.trim()
            ?.takeIf { it.isNotBlank() }
            ?.let { raw -> ParserUtils.toAbsoluteUrl(baseUrl, raw) }
            .orEmpty()
        val hasTagBlocklist = document.selectFirst("body")
            ?.attr("data-tag-blocklist")
            ?.trim()
            ?.isNotBlank() == true
        val figures = figureSelectors
            .asSequence()
    ...
    private fun parseFigure(
        ...
        val submissionUrl = ParserUtils.toAbsoluteUrl(
        ...
        return SubmissionThumbnail(
            ...
            isBlockedByTag = parseBlockedByTag(
                image = image,
                hasTagBlocklist = hasTagBlocklist,
            ),
        )
    ...
    private fun parseFolderGroups(
        ...
        root.children().forEach { child ->
            when {
                ...
                (child.tagName() == "div" && child.hasClass("default-folders")) || child.tagName() == "ul" -> {
                    val listNode = if (child.tagName() == "div") child.selectFirst("ul") else child
                    val folders = listNode?.select("li")
                        ?.mapNotNull { node -> parseFolder(node, currentUrl) }
                        .orEmpty()
    ...
    private fun parseFolder(
        ...
        val folderUrl = linkNode
            ?.attr("href")
            ?.trim()
            ?.takeIf { href -> href.isNotBlank() }
            ?.let { href -> ParserUtils.toAbsoluteUrl(currentUrl, href) }
            ?: currentUrl

        val activeByClass = node.hasClass("active") ||
        ...
        val activeByFolderId = extractFolderId(folderUrl)
            ?.let { folderId -> folderId == extractFolderId(currentUrl) }
            ?: false
    ...
    private fun isNextButton(node: Element): Boolean {
        ...
        val submitLabel = node.selectFirst("button, input[type=submit]")
            ?.let { submit ->
                submit.attr("value")
                    .ifBlank { submit.text() }
                    .trim()
    ...
    private fun buildGetFormUrl(
        ...
        val actionUrl = if (actionRaw.isBlank()) {
            baseUrl
        } else {
            ParserUtils.toAbsoluteUrl(baseUrl, actionRaw)
        }
        val params = formNode.select("input[name], select[name], textarea[name]")
            .mapNotNull { input ->
                ...
                if ((type == "checkbox" || type == "radio") && !input.hasAttr("checked")) return@mapNotNull null
