...
class SubmissionDescriptionTranslationService(
    ...
    fun extractBlocks(descriptionHtml: String): List<SubmissionDescriptionBlock> {
        ...
        val blocks = blockHtmlList
            .mapNotNull { html ->
                val sourceText = normalizeTranslationText(
                    htmlToString(
                        html = html,
                        compactMode = true,
                    ),
    ...
    suspend fun translateBlocks(
        ...
        coroutineScope {
            val resultChannel = Channel<Pair<Int, List<SubmissionDescriptionBlockResult>>>(
            ...
            repeat(chunks.size) {
                ...
                results.forEachIndexed { offset, result ->
                    onBlockResult(startIndex + offset, result)
                }
            }
            resultChannel.close()
        }
        ...
        val translatedLines = runCatching {
            translationPort.translate(
                TranslationRequest(
                    ...
                    sourceText = payload,
                    openAiConfig = settings.openAiTranslationConfig.takeIf {
    ...
    private fun parseChunkTranslation(
        translated: String,
        ...
        if (byLineBreak.size < expectedBlockCount) {
            return buildList {
                addAll(byLineBreak)
                repeat(expectedBlockCount - byLineBreak.size) {
                    add("")
                }
            }
        }
    ...
    companion object {
        private val standaloneBoundaryTags = setOf(
            "p",
            "blockquote",
            "li",
            "h1",
        ...
        private val wrapperContainerTags = setOf(
            "div",
            "section",
            "article",
            "code",
        ...
        private val cjkRegex = Regex("[\\u3400-\\u4DBF\\u4E00-\\u9FFF\\uF900-\\uFAFF\\u3040-\\u30FF\\uAC00-\\uD7AF]")
 ...
 * 描述块。
 */
data class SubmissionDescriptionBlock(
