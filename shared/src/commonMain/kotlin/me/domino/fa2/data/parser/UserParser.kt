 ...
 */
class UserParser {
    private val backgroundUrlRegex = Regex(
        pattern = """background(?:-image)?\s*:\s*url\((['"]?)(.*?)\1\)""",
        options = setOf(RegexOption.IGNORE_CASE),
    )
    private val watchlistCountRegex = Regex(
    ...
    fun parse(
        ...
        val displayName = document.selectFirst(".c-usernameBlock__displayName")
            ?.text()
            ?.trim()
            ?.takeIf { it.isNotBlank() }
            ?: username
        val avatarUrl = document.selectFirst("userpage-nav-avatar img")
            ?.attr("src")
            ?.trim()
            ?.takeIf { it.isNotBlank() }
        ...
        val registeredAt = document.selectFirst(".user-title span.popup_date")
            ?.attr("title")
            ?.trim()
            ?.takeIf { it.isNotBlank() }
            ?: document.selectFirst(".user-title span.popup_date")
                ?.text()
                ?.trim()
                .orEmpty()
                .ifBlank { "未知" }
        ...
        val profileBannerUrl = parseProfileBannerUrl(
            document = document,
            baseUrl = url,
        )
        val profileNode = document.selectFirst("section.userpage-layout-profile .section-body.userpage-profile")
    ...
    private fun parseUsername(document: com.fleeksoft.ksoup.nodes.Document): String {
        val fromHref = document.selectFirst("userpage-nav-avatar a[href*='/user/']")
            ?.attr("href")
            ?.substringAfter("/user/")
            ?.substringBefore('/')
            ?.trim()
        ...
        val fromLabel = document.selectFirst(".c-usernameBlock__userName")
            ?.text()
            ?.trim()
            .orEmpty()
            .removePrefix("~")
    ...
    private fun parseWatchState(
        ...
        val actionNode = document.selectFirst(
        ...
        val label = actionNode.text()
            ...
            .replace(" ", "")
            .lowercase()
        val isWatching = absoluteUrl.contains("/unwatch/") ||
                label.startsWith("-watch") ||
                label.contains("unwatch")
        return WatchState(
    ...
    private fun parseProfileBannerUrl(
        ...
        val fromDesktopBannerSource = document.selectFirst(
            "site-banner source[media*='min-width'][srcset]",
        )?.attr("srcset")
        ...
        val fromBannerImage = document.selectFirst(
        ...
        val fromAnyBannerSource = document.selectFirst(
            "site-banner source[srcset]",
        )?.attr("srcset")
            ?.let { raw -> extractSrcsetFirstUrl(raw) }
            ?.trim()
            .orEmpty()
        ...
        val inlineStyle = document.selectFirst("userpage-nav-header")
            ?.attr("style")
            ?.trim()
            .orEmpty()
        ...
        val fromEmbeddedStyle = document.select("style")
            .asSequence()
            .map { node -> node.html() }
            .filter { css -> css.contains("userpage-nav-header") && css.contains("background", ignoreCase = true) }
    ...
    private fun parseWatchlistInfo(
        ...
        val watchedByUrl = watchedByNode
            ?.attr("href")
            .orEmpty()
            .trim()
            .takeIf { it.isNotBlank() }
    ...
    private fun parseWatchlistCount(rawLabel: String): Int? {
        ...
        val fromPattern = watchlistCountRegex.find(text)
            ?.groupValues
            ?.getOrNull(1)
            ?.replace(",", "")
            ?.trim()
    ...
    private fun extractBackgroundUrl(cssLike: String): String? =
        backgroundUrlRegex.find(cssLike)
            ?.groupValues
            ?.getOrNull(2)
            ?.trim()
            ?.takeIf { it.isNotBlank() }
