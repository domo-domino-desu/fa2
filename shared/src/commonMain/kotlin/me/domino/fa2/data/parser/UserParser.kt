...
class UserParser {
  ...
  fun parse(html: String, url: String): User {
    ...
    val registeredAt =
      document.selectFirst(".user-title span.popup_date")?.attr("title")?.trim()?.takeIf {
        ...
        ?: document.selectFirst(".user-title span.popup_date")?.text()?.trim().orEmpty().ifBlank {
          "未知"
        }
  ...
  private fun extractBackgroundUrl(cssLike: String): String? =
    backgroundUrlRegex.find(cssLike)?.groupValues?.getOrNull(2)?.trim()?.takeIf { it.isNotBlank() }
