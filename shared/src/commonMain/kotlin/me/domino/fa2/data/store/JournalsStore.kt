...
class JournalsStore(
    ...
    fun stream(
        username: String,
        nextPageUrl: String?,
    ): Flow<PageState<JournalPage>> {
        val key = JournalsPageKey(
    ...
    private fun buildStore(): Store<JournalsPageKey, JournalPage> {
        val fetcher = Fetcher.of<JournalsPageKey, JournalPage>(name = "journals-fetcher") { key ->
            dataSource.fetchPage(key.username, key.nextPageUrl).requireStoreValue()
        }

        val sourceOfTruth = SourceOfTruth.of<JournalsPageKey, JournalPage, JournalPage>(
            reader = { key ->
    ...
    private fun parseJournalsPageKey(cacheKey: String): JournalsPageKey? {
        ...
        return JournalsPageKey(
            username = normalizeUsername(username),
            nextPageUrl = nextPageUrl,
        )
    }

    private data class JournalsPageKey(
        val username: String,
...
private fun Flow<PageState<JournalPage>?>.flowWithInitialLoading(): Flow<PageState<JournalPage>> = flow {
