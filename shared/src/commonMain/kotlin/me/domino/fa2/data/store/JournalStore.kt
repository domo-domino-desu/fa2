...
class JournalStore(
  ...
  fun streamById(journalId: Int): Flow<PageState<JournalDetail>> =
    store
      .stream(StoreReadRequest.cached(journalId, true))
      .map(::toPageState)
  ...
  private fun buildStore(): Store<Int, JournalDetail> {
    val fetcher =
      Fetcher.of<Int, JournalDetail>(name = "journal-fetcher") { journalId ->
        dataSource.fetchById(journalId).requireStoreValue()
      }

    val sourceOfTruth =
      SourceOfTruth.of<Int, JournalDetail, JournalDetail>(
        reader = { journalId ->
        ...
        writer = { journalId, detail -> writeCacheById(journalId, detail) },
        delete = { journalId -> pageCacheDao.delete(cacheKeyFor(journalId)) },
  ...
  private suspend fun writeCacheById(journalId: Int, detail: JournalDetail) {
    pageCacheDao.upsert(
      PageCacheEntity(
...
private fun Flow<PageState<JournalDetail>?>.flowWithInitialLoading():
  Flow<PageState<JournalDetail>> = flow {
