...
class JournalsParser {
    ...
    private fun parseJournalSummary(section: Element): JournalSummary? {
        ...
        val title = section.selectFirst("div.section-header h2")
            ?.text()
            ?.trim()
            .orEmpty()
            .ifBlank { "Untitled Journal #$sid" }
        ...
        val commentCount = section.selectFirst("div.section-footer span.font-large")
            ?.text()
            ?.filter { ch -> ch.isDigit() }
            ?.toIntOrNull()
            ?: 0

        val journalUrl = section.selectFirst("div.section-footer a[href*='/journal/']")
            ?.attr("href")
            ?.trim()
        ...
        val bodyText = section.selectFirst("div.section-body div.journal-body")
            ?.html()
