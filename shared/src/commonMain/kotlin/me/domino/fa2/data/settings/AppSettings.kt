...
data class AppSettings(
    ...
    val blockedSubmissionWaterfallMode: BlockedSubmissionWaterfallMode = defaultBlockedSubmissionWaterfallMode,
    val blockedSubmissionPagerMode: BlockedSubmissionPagerMode = defaultBlockedSubmissionPagerMode,
) {
    companion object {
        val supportedTranslationProviders: List<TranslationProvider> = listOf(
        ...
        val supportedBlockedSubmissionWaterfallModes: List<BlockedSubmissionWaterfallMode> = listOf(
            BlockedSubmissionWaterfallMode.SHOW,
            BlockedSubmissionWaterfallMode.BLUR_THEN_OPEN,
        )
        val supportedBlockedSubmissionPagerModes: List<BlockedSubmissionPagerMode> = listOf(
        ...
        fun normalize(raw: AppSettings): AppSettings =
            raw.copy(
                ...
                translationChunkWordLimit = raw.translationChunkWordLimit
                    .coerceIn(minTranslationChunkWordLimit, maxTranslationChunkWordLimit),
                translationMaxConcurrency = raw.translationMaxConcurrency
                    .coerceIn(minTranslationMaxConcurrency, maxTranslationMaxConcurrency),
                waterfallMinCardWidthDp = raw.waterfallMinCardWidthDp
                    .coerceIn(minWaterfallMinCardWidthDp, maxWaterfallMinCardWidthDp),
                blockedSubmissionWaterfallMode = raw.blockedSubmissionWaterfallMode
                    .takeIf { mode -> mode in supportedBlockedSubmissionWaterfallModes }
                    ?: defaultBlockedSubmissionWaterfallMode,
                blockedSubmissionPagerMode = raw.blockedSubmissionPagerMode
                    .takeIf { mode -> mode in supportedBlockedSubmissionPagerModes }
 ...
 * 瀑布流中的被屏蔽投稿显示策略。
 */
enum class BlockedSubmissionWaterfallMode(
    val persistedValue: String,
) {
    SHOW("show"),
    ...
    companion object {
        fun fromPersistedValue(raw: String?): BlockedSubmissionWaterfallMode? =
            entries.firstOrNull { it.persistedValue == raw?.trim() }
    }
}
 ...
 * 左右滑中的被屏蔽投稿策略。
 */
enum class BlockedSubmissionPagerMode(
    val persistedValue: String,
) {
    SHOW("show"),
    BLUR_THEN_OPEN("blur_then_open"),
    ...
    companion object {
        fun fromPersistedValue(raw: String?): BlockedSubmissionPagerMode? =
            entries.firstOrNull { it.persistedValue == raw?.trim() }
    }
}
