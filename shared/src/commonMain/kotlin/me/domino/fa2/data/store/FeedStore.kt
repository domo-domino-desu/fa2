...
class FeedStore(
    ...
    suspend fun loadPageByNextUrl(nextPageUrl: String): PageState<FeedPage> {
        val fromSid = ParserUtils.parseSubmissionsFromSid(nextPageUrl)
    ...
    suspend fun prefetchPage(fromSid: Int?) {
        val key = FeedPageKey(fromSid = fromSid)
        store.stream(StoreReadRequest.fresh(key, false))
            .first(::isTerminalResponse)
    }

    private fun buildStore(): Store<FeedPageKey, FeedPage> {
        val fetcher = Fetcher.of<FeedPageKey, FeedPage>(name = "feed-fetcher") { key ->
            dataSource.fetchPage(fromSid = key.fromSid).requireStoreValue()
        }

        val sourceOfTruth = SourceOfTruth.of<FeedPageKey, FeedPage, FeedPage>(
            reader = { key ->
        ...
        return StoreBuilder.from(fetcher = fetcher, sourceOfTruth = sourceOfTruth)
            .build()
    }
    ...
    private data class FeedPageKey(
        val fromSid: Int?,
    )
