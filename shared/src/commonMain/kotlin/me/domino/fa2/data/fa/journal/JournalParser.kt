...
import me.domino.fa2.data.fa.user.ensureUserPageAccessible
import me.domino.fa2.data.model.JournalDetail
import me.domino.fa2.data.model.PageComment
import me.domino.fa2.utils.parseJournalId
...
class JournalParser {
  ...
  fun parse(html: String, url: String): JournalDetail {
    ...
    val title =
        document.selectFirst("#c-journalTitleTop__subject h3")?.text()?.trim().orEmpty().ifBlank {
          "Untitled Journal"
        }
    ...
    val rating =
        document
            .selectFirst("#c-journalTitleTop__contentRating")
            ?.text()
            ?.trim()
            .orEmpty()
            .ifBlank { "Unknown" }
    ...
    val commentCount =
        document
            .selectFirst("div.content > section div.section-footer span.font-large")
            ?.text()
    ...
    val journalUrl =
        document.selectFirst("meta[property='og:url']")?.attr("content")?.trim()?.takeIf {
          it.isNotBlank()
        } ?: url
  ...
  private fun parseCommentPostingState(document: Document): CommentPostingState {
    ...
    if (!hasCommentForm && responseText.isNotBlank()) {
      return CommentPostingState(enabled = false, message = responseText)
    }
    return CommentPostingState(enabled = hasCommentForm)
  }

  private data class CommentPostingState(val enabled: Boolean, val message: String? = null)
}
