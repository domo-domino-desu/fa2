...
class SearchScreenModel(
    ...
    private val paginationStateMachine = PaginationStateMachine<SubmissionThumbnail, Int>(keyOf = { item -> item.id })
    ...
    fun toggleGender(gender: SearchGender, checked: Boolean) {
        ...
        val updatedGenders = current.selectedGenders.toMutableSet().apply {
            if (checked) add(gender) else remove(gender)
        }.toSet()
        val rewrittenQuery = rewriteQueryWithGenders(
            query = current.query,
    ...
    fun updateCategory(category: Int) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(category = category),
        )
    }

    fun updateType(type: Int) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(type = type),
        )
    }

    fun updateSpecies(species: Int) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(species = species),
        )
    }

    fun updateOrderBy(orderBy: String) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(orderBy = orderBy),
        )
    }

    fun updateOrderDirection(orderDirection: String) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(orderDirection = orderDirection),
        )
    }

    fun updateRange(range: String) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(range = range),
        )
    }

    fun updateRangeFrom(value: String) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(rangeFrom = value),
        )
    }

    fun updateRangeTo(value: String) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(rangeTo = value),
        )
    }

    fun setRatingGeneral(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(ratingGeneral = enabled),
        )
    }

    fun setRatingMature(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(ratingMature = enabled),
        )
    }

    fun setRatingAdult(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(ratingAdult = enabled),
        )
    }

    fun setTypeArt(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typeArt = enabled),
        )
    }

    fun setTypeMusic(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typeMusic = enabled),
        )
    }

    fun setTypeFlash(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typeFlash = enabled),
        )
    }

    fun setTypeStory(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typeStory = enabled),
        )
    }

    fun setTypePhoto(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typePhoto = enabled),
        )
    }

    fun setTypePoetry(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draft = state.value.draft.copy(typePoetry = enabled),
        )
    }

    fun applySearch() {
        log.i { "应用Search -> 开始" }
        val applied = state.value.draft.copy(
            query = state.value.draft.query.trim(),
        )
        if (applied.query.isBlank()) {
            log.w { "应用Search -> 跳过(空关键词)" }
            return
        }
        firstPageUrl = buildSearchUrl(
            form = applied,
            page = 1,
        )
        mutableState.value = state.value.copy(
    ...
    fun onLastVisibleIndexChanged(lastVisibleIndex: Int) {
        ...
        log.d {
            "自动加载Search -> 触发检查(last=$lastVisibleIndex,total=${snapshot.submissions.size})"
        }
    ...
    private fun load(forceRefresh: Boolean) {
        ...
        val snapshot = state.value
        mutableState.value = snapshot.applyPagination(
        ...
        loadJob = screenModelScope.launch {
            val pageState = if (forceRefresh) {
                repository.refreshPage(firstUrl)
            } else {
                repository.loadPage(firstUrl)
            }
            val reduced = paginationStateMachine.reduceFirstPage(
            ...
            when (pageState) {
                is PageState.Success -> {
                    ...
                    log.i { "加载Search -> ${summarizePageState(pageState)}(count=${updated.submissions.size})" }
                }
    ...
    private fun loadMore(force: Boolean) {
        ...
        appendJob = screenModelScope.launch {
            ...
            val reduced = paginationStateMachine.reduceAppend(
                snapshot = state.value.toPaginationSnapshot(),
                result = pageState,
                itemsOf = { page -> page.submissions },
                nextPageUrlOf = { page -> page.nextPageUrl },
            ...
            when (pageState) {
                is PageState.Success -> {
                    syncSubmissionListHolder(updated)
                    log.d { "自动加载Search -> ${summarizePageState(pageState)}(count=${updated.submissions.size})" }
                }
...
internal fun rewriteQueryWithGenders(
    query: String,
    genders: Set<SearchGender>,
): String {
    val tokens = query.split(Regex("\\s+")).filter { token -> token.isNotBlank() }.toMutableList()
    val keywordsIndex = tokens.indexOfFirst { token -> token.equals("@keywords", ignoreCase = true) }
    val selectedGenderTokens = SearchGender.entries
        .filter { gender -> gender in genders }
        .map { gender -> gender.token }
