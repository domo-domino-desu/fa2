...
internal fun UserSubmissionSectionScreen(
    ...
    val latestOnDeferredBodyScrollPositionConsumed = rememberUpdatedState(onDeferredBodyScrollPositionConsumed)
    val scrollLayout = remember(headerContent != null) {
    ...
    LaunchedEffect(waterfallState) {
        snapshotFlow {
            ...
            val sharedTopState = resolveUserSharedTopScrollState(
    ...
    LaunchedEffect(waterfallState, deferredBodyScrollPosition, scrollLayout) {
        val pendingBodyScrollPosition = deferredBodyScrollPosition?.takeUnless { it.isAtStart } ?: return@LaunchedEffect
        snapshotFlow {
        ...
        }.distinctUntilChanged().collect { sharedTopState ->
            if (sharedTopState == UserSharedTopScrollState.Sticky) {
    ...
    val topItemsContent: LazyStaggeredGridScope.() -> Unit = {
        ...
        if (state.folderGroups.isNotEmpty()) {
            item(
                key = "user-folder-groups",
                span = StaggeredGridItemSpan.FullLine,
            ) {
                UserFolderGroupsCard(
    ...
    val shouldStickTabs by remember(waterfallState, headerContent) {
        derivedStateOf {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        val blockingMessage = state.errorMessage
            ?.takeIf { state.submissions.isEmpty() && it.isNotBlank() }
        when {
            ...
            !blockingMessage.isNullOrBlank() -> {
                ...
                val inlineError = state.errorMessage
                    ?.takeIf { value -> value.isNotBlank() && state.submissions.isNotEmpty() }
...
private fun UserSubmissionSectionSingleStateGrid(
    ...
    LazyVerticalStaggeredGrid(
        ...
        preItemsContent()
        item(
            key = "user-submission-single-state-content",
            span = StaggeredGridItemSpan.FullLine,
        ) {
            content()
        }
...
@Composable
private fun UserFolderGroupsCard(
    groups: List<GalleryFolderGroup>,
    onOpenFolder: (String) -> Unit,
) {
    Column(
        modifier = Modifier
