package me.domino.fa2.ui.screen.submission
...
class SubmissionScreenModel(
    ...
    fun toggleFavoriteCurrent() {
        ...
        val optimisticDetail = originalDetail.copy(
            isFavorited = !originalDetail.isFavorited,
            favoriteCount = (originalDetail.favoriteCount + if (originalDetail.isFavorited) -1 else 1)
                .coerceAtLeast(0),
        )
        detailBySid[sid] = currentState.copy(
            ...
            when (val toggleResult = submissionSource.toggleFavorite(sid = sid, actionUrl = actionUrl)) {
                is PageState.Success -> {
                    ...
                    when (val refreshed = submissionSource.loadByUrl(targetUrl)) {
                        is PageState.Success -> {
                            ...
                            detailBySid[sid] = SubmissionDetailUiState.Success(
                                detail = optimisticDetail.copy(
                                    isFavorited = refreshed.data.isFavorited,
                                    favoriteCount = refreshed.data.favoriteCount,
                                    favoriteActionUrl = refreshed.data.favoriteActionUrl
                                        .ifBlank { guessNextFavoriteActionUrl(actionUrl) },
                                ),
                        ...
                        is PageState.Error -> {
                            detailBySid[sid] = SubmissionDetailUiState.Success(
                                ...
                                blockedKeywords = currentState.blockedKeywords,
                                favoriteUpdating = false,
                                favoriteErrorMessage = "收藏已提交，但状态同步失败：${refreshed.exception.message ?: refreshed.exception}",
                            )
                ...
                is PageState.Error -> {
                    detailBySid[sid] = SubmissionDetailUiState.Success(
                        ...
                        blockedKeywords = currentState.blockedKeywords,
                        favoriteUpdating = false,
                        favoriteErrorMessage = toggleResult.exception.message ?: toggleResult.exception.toString(),
                    )
        ...
        if (nonce.isBlank()) {
            screenModelScope.launch {
                ...
                when (val refreshed = submissionSource.loadByUrl(targetUrl)) {
                    is PageState.Success -> {
                        prefetchedSuccessSids += sid
                        val refreshedState = SubmissionDetailUiState.Success(
                        ...
                        if (refreshedNonce.isBlank()) {
                            detailBySid[sid] = refreshedState.copy(
                                favoriteErrorMessage = "当前页面缺少标签屏蔽凭据",
                            )
                            refreshState()
                            emitToast("缺少标签屏蔽凭据")
                    ...
                    PageState.CfChallenge -> {
                        detailBySid[sid] = currentState.copy(
                            favoriteErrorMessage = "刷新详情失败：需要 Cloudflare 验证",
                        )
                        refreshState()
                    ...
                    is PageState.MatureBlocked -> {
                        detailBySid[sid] = currentState.copy(
                            favoriteErrorMessage = "刷新详情失败：${refreshed.reason}",
                        )
                        refreshState()
                    ...
                    is PageState.Error -> {
                        ...
                        detailBySid[sid] = currentState.copy(
                            favoriteErrorMessage = "刷新详情失败：$message",
                        )
                        refreshState()
    ...
    private suspend fun executeTagBlockRequest(
        ...
        when (
            val blocked = submissionSource.blockTag(
                sid = sid,
                tagName = tagName,
                nonce = nonce,
                toAdd = toAdd,
            ...
            is PageState.MatureBlocked -> {
                val latest = detailBySid[sid] as? SubmissionDetailUiState.Success ?: return
                detailBySid[sid] = latest.copy(
                    favoriteErrorMessage = blocked.reason,
                )
    ...
    private fun refreshState() {
        ...
        mutableState.value = SubmissionPagerUiState.Data(
            submissions = items,
    ...
    private fun computePrefetchCandidateSids(): List<Int> {
        ...
        val targetIndices = computeSubmissionPrefetchIndices(
    ...
    private fun appendIfNearEnd(force: Boolean) {
        ...
        log.d {
            "自动加载投稿列表 -> 触发检查(force=$force,current=${holder.currentIndex()},size=$currentSize)"
        }
        ...
        applyPaginationSnapshot(
            paginationStateMachine.beginAppend(snapshot),
        )
        refreshState()
    ...
    private fun loadDetail(
        ...
        screenModelScope.launch {
            val targetUrl = current.submissionUrl.ifBlank { FaUrls.submission(current.id) }
            val detailState = when (val next = submissionSource.loadByUrl(targetUrl)) {
                is PageState.Success -> {
                    prefetchedSuccessSids += sid
...
private fun normalizeTagKey(tagName: String): String =
    tagName.trim().lowercase()
