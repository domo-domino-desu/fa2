...
class BrowseScreenModel(
    ...
    fun updateCategory(category: Int) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(category = category),
        )
    }

    fun updateType(type: Int) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(type = type),
        )
    }

    fun updateSpecies(species: Int) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(species = species),
        )
    }

    fun updateGender(gender: String) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(gender = gender),
        )
    }

    fun setRatingGeneral(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(ratingGeneral = enabled),
        )
    }

    fun setRatingMature(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(ratingMature = enabled),
        )
    }

    fun setRatingAdult(enabled: Boolean) {
        mutableState.value = state.value.copy(
            draftFilter = state.value.draftFilter.copy(ratingAdult = enabled),
        )
    }
    ...
    fun applyFilter() {
        log.i { "应用Browse筛选 -> 开始" }
        mutableState.value = state.value.copy(
            appliedFilter = state.value.draftFilter,
        )
    ...
    private fun loadMore(force: Boolean) {
        ...
        appendJob = screenModelScope.launch {
            ...
            when (pageState) {
                is PageState.Success -> {
                    syncSubmissionListHolder(updated)
                    log.d { "自动加载Browse -> ${summarizePageState(pageState)}(count=${updated.submissions.size})" }
                }
