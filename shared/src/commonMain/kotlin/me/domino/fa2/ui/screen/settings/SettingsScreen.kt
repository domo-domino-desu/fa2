...
import me.domino.fa2.ui.component.topbar.SettingsRouteTopBar
...
fun SettingsScreen(
    ...
    fun saveDraft() {
        ...
        scope.launch {
            runCatching {
                settingsService.updateSettings(target)
            }.onSuccess {
                saveStatusText = "设置已保存"
            }.onFailure { error ->
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            saveStatusText?.takeIf { it.isNotBlank() }?.let { message ->
                item {
                    Text(
                        text = message,
                        style = MaterialTheme.typography.bodySmall,
