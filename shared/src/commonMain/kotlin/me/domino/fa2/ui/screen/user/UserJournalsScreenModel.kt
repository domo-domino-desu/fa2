...
class UserJournalsScreenModel(
    ...
    private val paginationStateMachine = PaginationStateMachine<JournalSummary, Int>(keyOf = { item -> item.id })
    ...
    fun onLastVisibleIndexChanged(lastVisibleIndex: Int) {
        ...
        log.d {
            "自动加载Journals页 -> 触发检查(last=$lastVisibleIndex,total=${snapshot.journals.size})"
        }
