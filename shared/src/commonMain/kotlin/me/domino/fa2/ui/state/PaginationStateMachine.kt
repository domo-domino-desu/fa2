...
class PaginationStateMachine<Item, Key>(
    ...
    fun <Page> reduceFirstPage(
        ...
        when (result) {
            is PageState.Success -> snapshot.copy(
            ...
            PageState.CfChallenge -> snapshot.copy(
                loading = false,
                refreshing = false,
                isLoadingMore = false,
            ...
            is PageState.MatureBlocked -> snapshot.copy(
                loading = false,
                refreshing = false,
                isLoadingMore = false,
            ...
            is PageState.Error -> snapshot.copy(
                loading = false,
                refreshing = false,
                isLoadingMore = false,
    ...
    fun <Page> reduceAppend(
        ...
        when (result) {
            ...
            PageState.CfChallenge -> snapshot.copy(
                isLoadingMore = false,
                appendErrorMessage = challengeMessage,
            )

            is PageState.MatureBlocked -> snapshot.copy(
                isLoadingMore = false,
                appendErrorMessage = result.reason,
            )
            ...
            PageState.Loading -> snapshot.copy(
                isLoadingMore = false,
                appendErrorMessage = appendFallbackErrorMessage,
            )
        }

    private fun mergeByKey(
        existing: List<Item>,
        incoming: List<Item>,
    ): List<Item> {
