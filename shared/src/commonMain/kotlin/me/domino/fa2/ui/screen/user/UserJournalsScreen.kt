...
internal fun UserJournalsScreen(
    ...
    LaunchedEffect(listState) {
        snapshotFlow {
            listState.layoutInfo.visibleItemsInfo.maxOfOrNull { info -> info.index } ?: 0
        }.distinctUntilChanged().collect { lastVisible ->
            latestOnLastVisible.value(lastVisible)
        }
    }
    ...
    val shouldStickTabs by remember(listState, headerContent) {
        derivedStateOf {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        PullToRefreshBox(
            ...
            LazyColumn(
                ...
                item(key = "user-tabs-inline") {
                    tabsContent(Modifier)
                }

                when {
                    state.loading && state.journals.isEmpty() -> {
                        item(key = "journals-skeleton") {
                            UserJournalsSkeleton()
                        }
                    }
                    ...
                    !state.errorMessage.isNullOrBlank() && state.journals.isEmpty() -> {
                        ...
                        val inlineError = state.errorMessage
                            ?.takeIf { value -> value.isNotBlank() && state.journals.isNotEmpty() }
        ...
        Row(
            ...
            if (hasMore && !isLoadingMore) {
                AssistChip(
                    ...
                    label = {
                        Text(
                            text = if (appendErrorMessage.isNullOrBlank()) "加载下一页" else "手动加载",
                        )
                    },
                )
            }
...
@Composable
private fun UserJournalsStatusCard(
    title: String,
    body: String,
    onRetry: () -> Unit,
) {
