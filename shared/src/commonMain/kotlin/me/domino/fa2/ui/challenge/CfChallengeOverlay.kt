...
import me.domino.fa2.ui.screen.auth.CfChallengeWebView
import me.domino.fa2.ui.screen.auth.rememberChallengeWebViewAdapter
...
fun CfChallengeOverlay(
    ...
    LaunchedEffect(state.triggerUrl) {
        controller.prepareWebViewSession(
            port = adapter.port,
            triggerUrl = state.triggerUrl,
        )
    }

    LaunchedEffect(adapter.port.lastLoadedUrl) {
        controller.syncUserAgentFromWebView(adapter.port)
    }
    ...
    Surface(
        ...
        Column(
            ...
            Row(
                ...
                Button(
                    onClick = {
                        adapter.port.loadUrl(state.triggerUrl)
                    },
                ) {
                    Text("重载")
                }
