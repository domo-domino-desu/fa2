...
class SubmissionRouteScreen(
    ...
    override fun Content() {
        ...
        val screenModel = koinScreenModel<SubmissionScreenModel> {
            parametersOf(initialSid, submissionListHolder)
        }
        ...
        val requestPagerFocus = remember(focusRequester) {
            {
                focusRequester.requestFocus()
                Unit
            }
        }
        val topBarActions = resolveTopBarActions(
        ...
        LaunchedEffect(screenModel, showToast) {
            screenModel.toastMessages.collect { message ->
                showToast(message)
            }
        }
        val currentHistoryItem = when (val snapshot = state) {
        ...
        LaunchedEffect(currentHistoryItem?.id) {
            currentHistoryItem?.let { item ->
                historyRepository.recordSubmissionVisit(item)
            }
        }
                    ...
                    Box(modifier = Modifier.fillMaxSize()) {
                        SubmissionPager(
                            ...
                            onSearchKeyword = { query ->
                                ...
                                if (normalized.isNotBlank()) {
                                    navigator.push(
                                        SearchRouteScreen(initialQuery = normalized),
                                    )
                                }
                            },
                        ...
                        val detailState = currentItem
                            ?.let { item -> snapshot.detailBySid[item.id] as? SubmissionDetailUiState.Success }
                        if (!zoomOverlayVisible && detailState != null && detailState.detail.favoriteActionUrl.isNotBlank()) {
                            FloatingActionButton(
                                ...
                                containerColor = if (detailState.detail.isFavorited) {
                                ...
                                if (detailState.favoriteUpdating) {
                                    ...
                                    Icon(
                                        imageVector = if (detailState.detail.isFavorited) {
                                            Icons.Filled.Favorite
                                        } else {
                                            Icons.Filled.FavoriteBorder
                                        },
                                        contentDescription = if (detailState.detail.isFavorited) {
                                            "Unfavorite"
                                        } else {
                                            "Favorite"
                                        },
...
private data class SubmissionTopBarActions(
    val shareUrl: String,
...
private fun resolveTopBarActions(
    ...
    return when (state) {
        SubmissionPagerUiState.Empty -> SubmissionTopBarActions(
            shareUrl = FaUrls.submission(initialSid),
            downloadUrl = null,
        )
...
@Composable
private fun SubmissionImagePrefetchEffect(
    snapshot: SubmissionPagerUiState.Data,
) {
    ...
    LaunchedEffect(snapshot.currentIndex, snapshot.submissions, snapshot.detailBySid) {
        ...
        targetIndices.forEach { index ->
            ...
            val derivedThumbnailUrl = (detailState as? SubmissionDetailUiState.Success)
                ?.detail
                ?.fullImageUrl
                ?.let { fullImageUrl ->
