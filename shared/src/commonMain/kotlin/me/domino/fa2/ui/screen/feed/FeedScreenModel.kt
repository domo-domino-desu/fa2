...
class FeedScreenModel(
    ...
    fun load(forceRefresh: Boolean = false) {
        ...
        mutableState.value = current.applyPagination(
        ...
        loadJob = screenModelScope.launch {
            ...
            when (val next = firstPageState) {
                ...
                PageState.CfChallenge -> log.w { "加载Feed -> Cloudflare验证" }
                is PageState.MatureBlocked -> log.w { "加载Feed -> 受限(${next.reason})" }
                is PageState.Error -> log.e(next.exception) { "加载Feed -> 失败" }
                PageState.Loading -> log.d { "加载Feed -> 加载中" }
            }
        }
    }
