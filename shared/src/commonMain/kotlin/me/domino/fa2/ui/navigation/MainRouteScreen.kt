...
class MainRouteScreen(
    ...
    override fun Content() {
        ...
        val feedSubmissionListHolder = navigator.rememberNavigatorScreenModel<SubmissionListHolder>(
            tag = "submission-list-holder:feed",
        ) {
            SubmissionListHolder()
        }
        val browseSubmissionListHolder = navigator.rememberNavigatorScreenModel<SubmissionListHolder>(
            tag = "submission-list-holder:browse",
        ) {
            SubmissionListHolder()
        }
        val searchSubmissionListHolder = navigator.rememberNavigatorScreenModel<SubmissionListHolder>(
            tag = "submission-list-holder:search",
        ) {
            SubmissionListHolder()
        }
        val feedScreenModel = koinScreenModel<FeedScreenModel> {
            parametersOf(feedSubmissionListHolder)
        }
        val browseScreenModel = koinScreenModel<BrowseScreenModel> {
            parametersOf(browseSubmissionListHolder)
        }
        val searchScreenModel = koinScreenModel<SearchScreenModel> {
            parametersOf(searchSubmissionListHolder)
        }
        ...
        val topLevelDestinationHolder = navigator.rememberNavigatorScreenModel<MainTopLevelDestinationHolder>(
            ...
            onTopLevelDestinationClick = { destination, reselected ->
                ...
                when (destination) {
                    TopLevelDestination.FEED -> {
                        val atTop = feedWaterfallState.firstVisibleItemIndex == 0 &&
                                feedWaterfallState.firstVisibleItemScrollOffset == 0
                        if (atTop) {
                            feedScreenModel.refresh()
                        } else {
                            coroutineScope.launch {
                                feedWaterfallState.animateScrollToItem(0)
                            }
                        }
                    }

                    TopLevelDestination.BROWSE -> {
                        val atTop = browseWaterfallState.firstVisibleItemIndex == 0 &&
                                browseWaterfallState.firstVisibleItemScrollOffset == 0
                        if (atTop) {
                            browseScreenModel.refresh()
                        } else {
                            coroutineScope.launch {
                                browseWaterfallState.animateScrollToItem(0)
                            }
                        }
                    }
                    ...
                    TopLevelDestination.SEARCH -> {
                        coroutineScope.launch {
                            searchWaterfallState.animateScrollToItem(0)
                        }
                    }
            ...
            when (currentTopLevelDestination) {
                ...
                TopLevelDestination.MORE -> {
                    MoreScreen(
                        ...
                        onOpenSettings = {
                            navigator.push(SettingsRouteScreen())
                        },
                        onOpenSubmissionHistory = {
                            navigator.push(SubmissionHistoryRouteScreen())
                        },
                        onOpenSearchHistory = {
                            navigator.push(SearchHistoryRouteScreen())
                        },
                        onOpenAbout = {
                            navigator.push(AboutRouteScreen())
                        },
