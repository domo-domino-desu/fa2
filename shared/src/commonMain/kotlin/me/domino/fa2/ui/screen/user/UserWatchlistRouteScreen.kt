...
import me.domino.fa2.ui.component.topbar.UserWatchlistRouteTopBar
...
class UserWatchlistRouteScreen(
    ...
    override fun Content() {
        ...
        val screenModel = koinScreenModel<UserWatchlistScreenModel> {
            parametersOf(username, category, initialUrl)
        }
        val state by screenModel.state.collectAsState()
        val listState: LazyListState = rememberLazyListState()
        val latestOnLastVisible = rememberUpdatedState(screenModel::onLastVisibleIndexChanged)
        val shareUrl = when (category) {
        ...
        LaunchedEffect(listState) {
            snapshotFlow {
                ...
                .collect { lastIndex ->
                    latestOnLastVisible.value(lastIndex)
                }
        }
        val topBarTitle = when (category) {
                    ...
                    when {
                        ...
                        !state.errorMessage.isNullOrBlank() && state.users.isEmpty() -> {
                            ...
                            val inlineError = state.errorMessage
                                ?.takeIf { value -> value.isNotBlank() && state.users.isNotEmpty() }
...
@Composable
private fun WatchlistStatusCard(
    title: String,
    body: String,
    onRetry: () -> Unit,
) {
