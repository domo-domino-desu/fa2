...
/**
 * FA 站内链接在应用内打开（push 新页面）；
 * 其他链接透传给系统处理器。
 */
class FaLinkUriHandler(
...
private fun resolveFaTarget(url: String): Screen? {
    ...
    if (journalId != null) {
        return JournalDetailRouteScreen(
            journalId = journalId,
            journalUrl = url,
        )
    }
    ...
    return when {
        path.startsWith("/gallery/") -> extractUsername(path, "/gallery/")?.let { username ->
            UserRouteScreen(
                username = username,
        ...
        path.startsWith("/favorites/") -> extractUsername(path, "/favorites/")?.let { username ->
            UserRouteScreen(
                username = username,
        ...
        path.startsWith("/scraps/") -> extractUsername(path, "/scraps/")?.let { username ->
            UserRouteScreen(
                username = username,
                initialChildRoute = UserChildRoute.Gallery,
        ...
        path.startsWith("/journals/") -> extractUsername(path, "/journals/")?.let { username ->
            UserRouteScreen(username = username, initialChildRoute = UserChildRoute.Journals)
        }

        path.startsWith("/user/") -> extractUsername(path, "/user/")?.let { username ->
...
private fun extractInitialScrapsUrl(
    url: String,
    path: String,
): String =
    url.trim().takeIf { it.isNotBlank() }
        ?: ParserUtils.toAbsoluteUrl(FaUrls.home, path)
