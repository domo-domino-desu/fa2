...
internal data class UserScrollLayout(
...
internal fun userJournalsScrollLayout(hasHeader: Boolean): UserScrollLayout =
    UserScrollLayout(
        hasHeader = hasHeader,
        bodyStartIndex = if (hasHeader) 2 else 1,
    )
...
internal fun resolveUserBodyScrollPosition(
    ...
    val relativeOffset = if (firstVisibleItemIndex < layout.bodyStartIndex) 0 else firstVisibleItemScrollOffset
...
internal fun resolveInitialUserScrollPosition(
    ...
    layout: UserScrollLayout,
): UserResolvedScrollPosition = when (sharedTopScrollState) {
    is UserSharedTopScrollState.Inline -> UserResolvedScrollPosition(
    ...
    UserSharedTopScrollState.Sticky -> {
        ...
        UserResolvedScrollPosition(
            firstVisibleItemIndex = layout.bodyStartIndex + targetBodyScrollPosition.firstVisibleItemIndex,
...
internal fun resolveUserSectionTabSelectionAction(
    ...
    isAtTop: Boolean,
): UserSectionTabSelectionAction = when {
