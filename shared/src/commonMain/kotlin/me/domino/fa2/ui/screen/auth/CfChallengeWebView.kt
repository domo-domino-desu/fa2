...
fun rememberChallengeWebViewAdapter(initialUrl: String): ChallengeWebViewAdapter {
    ...
    return remember(webViewState, navigator) {
        ChallengeWebViewAdapter(
            port = ComposeCfChallengeWebViewPort(
 ...
 */
@Composable
fun CfChallengeWebView(
    adapter: ChallengeWebViewAdapter,
    modifier: Modifier,
) {
    WebView(
        state = adapter.webViewState,
        navigator = adapter.navigator,
        modifier = modifier,
    )
}
...
private class ComposeCfChallengeWebViewPort(
     ...
     */
    override suspend fun injectCookieHeader(
        url: String,
        cookieHeader: String,
    ) {
