...
fun rememberSubmissionDescriptionTranslationState(
    ...
    val sourceBlocks = remember(descriptionHtml) {
        service.extractBlocks(descriptionHtml)
    }

    var blockStates by remember(descriptionHtml) {
        mutableStateOf(sourceBlocks.map { block ->
    ...
    DisposableEffect(descriptionHtml) {
        onDispose {
            runningJob?.cancel()
        }
    }
    ...
    val translateAction: () -> Unit = translateAction@{
        ...
        translating = true
        hasTriggered = true
        blockStates = sourceBlocks.map { block ->
            SubmissionDescriptionBlockState(
                block = block,
                translated = null,
        ...
        runningJob = scope.launch {
            try {
                service.translateBlocks(sourceBlocks) { index, result ->
                    blockStates = blockStates.mapIndexed { currentIndex, state ->
                        if (currentIndex != index) return@mapIndexed state

                        when (result) {
                            is SubmissionDescriptionBlockResult.Success -> state.copy(
                                translated = result.translatedText,
                                status = SubmissionDescriptionTranslationStatus.SUCCESS,
                            )

                            SubmissionDescriptionBlockResult.EmptyResult -> state.copy(
                                translated = null,
                                status = SubmissionDescriptionTranslationStatus.EMPTY,
                            )

                            is SubmissionDescriptionBlockResult.Failure -> state.copy(
                                translated = null,
                ...
                blockStates = blockStates.map { state ->
    ...
    val displayBlocks = remember(blockStates, hasTriggered) {
        if (!hasTriggered) {
