...
fun androidPlatformModule(
    ...
    single<AppDatabaseBuilderFactory> {
        ...
        val cacheDbFile = File(appContext.cacheDir, "room-cache/fa2-cache.db").apply {
            parentFile?.mkdirs()
        }
    ...
    single<DataStore<Preferences>> {
        ...
        PreferenceDataStoreFactory.createWithPath(
            produceFile = {
                File(appContext.filesDir, "datastore/fa2.preferences_pb").apply {
                    parentFile?.mkdirs()
                }.absolutePath.toPath()
            },
        )
    ...
    single(named(KOIN_QUALIFIER_COOKIE_VAULT)) {
        KSafe(
            ...
            fileName = KOIN_QUALIFIER_COOKIE_VAULT,
        )
    }
}
