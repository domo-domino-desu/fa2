...
android {
    ...
    val signingStoreFilePath = providers
    ...
    val signingStorePassword = providers
        .gradleProperty("ANDROID_SIGNING_STORE_PASSWORD")
        .orElse("123456")
        .get()
    val signingKeyAlias = providers
        .gradleProperty("ANDROID_SIGNING_KEY_ALIAS")
        .orElse("dummy")
        .get()
    val signingKeyPassword = providers
        .gradleProperty("ANDROID_SIGNING_KEY_PASSWORD")
        .orElse("123456")
        .get()
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add(layout.buildDirectory.dir("generated/aboutlibrariesAssets").get().asFile.path)
        }
    }
...
val copyAboutLibrariesJsonToAssets by tasks.registering(Copy::class) {
    from(project(":shared").layout.projectDirectory.file("src/commonMain/composeResources/files/aboutlibraries.json"))
