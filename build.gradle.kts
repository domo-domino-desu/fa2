...
/**
 * Resolve the first executable command from candidates in PATH.
 * Useful for CI images where ImageMagick may expose `convert` (v6) instead of `magick` (v7).
 */
...
tasks.register("generateAppIcons") {
    ...
    val desktopIconDir = layout.projectDirectory.dir("desktopApp/src/desktopMain/resources/icon").asFile
        ...
        densities.forEach { (density, size) ->
            ...
            runCommand(
                listOf(
                    ...
                    "-h", size.toString(),
                    "-b", logoBackground,
                    "-o", dir.resolve("ic_launcher.png").absolutePath,
            ...
            runCommand(
                listOf(
                    ...
                    "-w", fgSize.toString(),
                    "-h", fgSize.toString(),
                    "-o", dir.resolve("ic_launcher_foreground.png").absolutePath,
                    logoSvg.absolutePath,
                ),
            )
        }
        ...
        iconSizes.forEach { size ->
            runCommand(
                listOf(
                    ...
                    "-h", size.toString(),
                    "-b", logoBackground,
                    "-o", desktopIconDir.resolve("icon-$size.png").absolutePath,
                    logoSvg.absolutePath,
                ),
            )
        }
